...
def register(app: typer.Typer) -> None:
    ...
    def validate_cmd(
        ...
        heading: Annotated[
            float | None,
            typer.Option(
                "--heading", help="Override satellite heading (deg, clockwise from north)."
            ),
        ] = None,
        incidence: Annotated[
            float | None,
    ...
    def sweep_cmd(
        ...
        grid: Annotated[
            Path, typer.Option("--grid", help="sweep.yaml (grid of dotted config keys).")
        ],
        out: Annotated[
            Path | None,
        ...
        radius: Annotated[
            float, typer.Option("--radius", help="Site averaging radius (m).")
        ] = 100.0,
        set_: Annotated[
            list[str] | None,
            typer.Option(
        ...
        gn = (
            gnss
            if gnss is not None
            else (cfg.validation.gnss.path if cfg.validation.gnss else None)
        )
        gt = None
    ...
    def closure_cmd(
        ...
        _require(igrams, "validate.cli.ts_not_found")
        _require(unw, "validate.cli.ts_not_found")
        ...
        paths: dict[str, str] = {}
        if out is not None:
            paths["json"] = mask_text(
                str(write_dashboard_json(result, out / "closure_dashboard.json"))
            )
