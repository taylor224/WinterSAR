...
| KB | 원본 문자열 | 출처 |
|---|---|---|
| KB-SNAPHU-001 | `Exceeded maximum number of secondary nodes\nDecrease TILECOSTTHRESH and/or increase MINREGIONSIZE\n` | gmgunter/snaphu `src/snaphu_tile.c` TraceRegions (snaphu-py `ext/snaphu` 서브모듈). **플랜의 "secondary arcs" 문구는 소스에 없음**(변수명에만 존재) → 패턴은 "nodes" 만 사용, 구버전 문구는 open question #22 |
